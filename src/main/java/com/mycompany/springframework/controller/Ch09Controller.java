...
public class Ch09Controller {
	...
	/*	@PostMapping("/fileUpload")
		...
		public void downloadFile(String fileName, 
			...
			String fileType = request.getServletContext().getMimeType(fileName); //image/jpg
			//한글로 되어있는 파일 이름 => ISO-8859-1 문자셋으로 구성된 파일이름으로 변환
			fileName = new String(fileName.getBytes("UTF-8"), "ISO-8859-1"); //반드시 들어가야함! (한글 이름 파일)
			
			
			//응답 헤더에 저장할 내용
			response.setContentType(fileType);
			response.setHeader("Content-Disposition", "attachment; filename=\"" + fileName +  "\""); //문자열+변수+문자열
